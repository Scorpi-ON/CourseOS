...
class Superblock(Entity):
    ...
    def _get_attrs(cls) -> tuple[str, ...]:
        return "fs_type", "block_size", "block_count", "inode_count", "free_block_count", "free_inode_count"

    @property
    def _struct_fmt(self) -> str:  # First extra byte is needed
    ...
    def __init__(
        ...
        assert 1 <= len(fs_type) <= conf.MAX_BYTE_VALUE
        assert inode_count % conf.BITS_IN_BYTE == 0
    ...
    def write(self, buf: typing.BinaryIO, pos: int | None = None) -> None:
        tools.pack(
            self._struct_fmt,
            buf,
            pos,
