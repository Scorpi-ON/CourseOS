...
class Bitmap(Block):
    ...
    def _bits_to_bytes(bits: bytes | bytearray) -> bytearray:
        ...
        for num in range(0, len(bits) - 1, conf.BITS_IN_BYTE):
            bytes_.append(int("".join(map(str, bits[num : num + conf.BITS_IN_BYTE])), 2))
        return bytes_
    ...
    def _index_helper(self, index: int) -> tuple[int, int]:
    ...
    def get_free_item_nums(self, count: int = 1) -> list[int]:
