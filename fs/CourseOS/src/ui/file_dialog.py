...
from CourseOS.src.entities.dynamic.file import File
...
class FileDialog(QDialog):
    ...
    def __init__(
        ...
        assert (file and inode and file_content is not None) or (not file and not inode and file_content is None)
        self.drive = drive
        self.file = file
        self.inode = inode
    ...
    @file_content.setter
    def file_content(self, value: bytes) -> None:
    ...
    @rights.setter
    def rights(self, value: Rights) -> None:
        self.suidChb.setChecked(value.suid == 1)
        self.sgidChb.setChecked(value.sgid == 1)
        for i in range(conf.TRIADE_SIZE):
            for j in range(conf.TRIADE_SIZE):
                self.rightsTable.item(i, j).setCheckState(
                    Qt.CheckState.Checked if value[2 + i * 3 + j] == 1 else Qt.CheckState.Unchecked
                )

    def saveBtn_enabler(self) -> None:
    ...
    def fill(self, filename: str, inode: Inode, file_content: bytes) -> None:
