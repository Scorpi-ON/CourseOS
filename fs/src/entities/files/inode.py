...
class Inode(Entity):
    ...
    def _get_attrs(cls) -> tuple[str, ...]:
        return "rights", "user_id", "group_id", "file_size", "ctime", "mtime", "address_array"
    ...
    def address_array(self, value: list[int]) -> None:
    ...
    def write(self, buf: BinaryIO, pos: int | None = None) -> None:
        ...
        tools.pack(Inode._TAIL_STRUCT_FMT, buf, None, self.user_id, self.group_id, self.file_size, *self.address_array)

    @classmethod
    def read(cls, buf: BinaryIO, pos: int | None = None) -> "Inode":
        ...
        return Inode(rights, user_id, group_id, file_size, ctime, mtime, list(address_array))

    def check_rights(self, user: User | None) -> tuple[int, int, int] | None:
        if user is None or (self.group_id != user.group_id and user.group_id == conf.SYSTEM_USER_AND_GROUP_ID):
            return 1, 1, 1
        if self.user_id == conf.SYSTEM_USER_AND_GROUP_ID:
            return None
        if self.user_id == user.id:
            return self.rights.user_read, self.rights.user_write, self.rights.user_execute
        if self.group_id == user.group_id:
